...
class AppService {
  ...
  Future<bool> login(HttpClient httpClient, Cookie? iPlanetDirectoryPro) async {
    ...
    request = await httpClient.getUrl(Uri.parse(
        "https://zjuam.zju.edu.cn/cas/login?service=http%3A%2F%2Fappservice.zju.edu.cn%2F")).timeout(const Duration(seconds: 8), onTimeout: () => throw ExceptionWithMessage("请求超时"));
  ...
  Future<Tuple<Exception?,Iterable<ExamDto>>> getExamsDto(
    ...
    try {
      ...
      var exception = e is SocketException
          ...
          : e as Exception;
      return Tuple(exception, (jsonDecode((_db.getCachedWebPage('appService_examDto_$xn-$xq') ?? '[]')
          .replaceAll(RegExp(r'\((?=[\u4e00-\u9fa5])'), '（')
          .replaceAll(RegExp(r'(?<=[\u4e00-\u9fa5])\)'), '）'))
      as List<dynamic>)
          .map((e) => ExamDto(e)));
  ...
  Future<Tuple<Exception?,Iterable<Session>>> getTimetable(HttpClient httpClient) async {
    late HttpClientRequest request;
    late HttpClientResponse response;
    ...
    try {
      ...
      var exception = e is SocketException
          ? ExceptionWithMessage("网络错误")
          : e as Exception;
      return Tuple(exception, (jsonDecode((_db.getCachedWebPage('appService_Timetable') ?? '[]')
          .replaceAll(RegExp(r'\((?=[\u4e00-\u9fa5])'), '（')
          .replaceAll(RegExp(r'(?<=[\u4e00-\u9fa5])\)'), '）'))
      as List<dynamic>)
          .where((e) => e['kcid'] != null)
