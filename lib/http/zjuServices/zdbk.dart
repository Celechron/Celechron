...
class Zdbk {
  ...
  Future<bool> login(HttpClient httpClient, Cookie? iPlanetDirectoryPro) async {
    ...
    request = await httpClient.getUrl(Uri.parse(
        "https://zjuam.zju.edu.cn/cas/login?service=http%3A%2F%2Fzdbk.zju.edu.cn%2Fjwglxt%2Fxtgl%2Flogin_ssologin.html")).timeout(const Duration(seconds: 8), onTimeout: () => throw ExceptionWithMessage("请求超时"));
    ...
    if (response.cookies
        .any((element) => element.name == 'route')) {
      _route = response.cookies
          .firstWhere((element) => element.name == 'route');
    } else {
  ...
  Future<Tuple<Exception?,List<double>>> getMajorGrade(
    ...
    try {
      if(_jSessionId == null || _route == null) throw ExceptionWithMessage("未登录");
      request = await httpClient
          .getUrl(Uri.parse("http://zdbk.zju.edu.cn/jwglxt/zycjtj/xszgkc_cxXsZgkcIndex.html?doType=query&queryModel.showCount=5000")).timeout(
      ...
      var transcriptJson = RegExp('(?<="items":)\\[(.*?)\\](?=,"limit")').firstMatch(await response.transform(utf8.decoder).join())?.group(0);
      ...
      var grades = (jsonDecode(_db.getCachedWebPage('zdbk_Transcript') ?? '[]') as List<dynamic>)
          .where((e) => e['xkkh'] != null)
          .map((e) => Grade(e));
      var majorGpa = GpaHelper.calculateGpa(grades);
  ...
  Future<Tuple<Exception?,Iterable<Grade>>> getTranscript(
    ...
    try {
      if(_jSessionId == null || _route == null) throw ExceptionWithMessage("未登录");
      request = await httpClient
          .getUrl(Uri.parse("http://zdbk.zju.edu.cn/jwglxt/cxdy/xscjcx_cxXscjIndex.html?doType=query&queryModel.showCount=5000")).timeout(
          const Duration(seconds: 8),
          onTimeout: () => throw ExceptionWithMessage("请求超时"));
      request.cookies.add(_jSessionId!);
      request.cookies.add(_route!);
      ...
      var grades = (jsonDecode(transcriptJson)
      as List<dynamic>)
          .where((e) => e['xkkh'] != null)
          .map((e) => Grade(e));
      _db.setCachedWebPage('zdbk_Transcript', transcriptJson);
      ...
      var exception = e is SocketException
          ? ExceptionWithMessage("网络错误") : e as Exception;
      return Tuple(exception, (jsonDecode((_db.getCachedWebPage('zdbk_Transcript') ?? '[]'))
          as List<dynamic>)
          .where((e) => e['xkkh'] != null)
          .map((e) => Grade(e)));
    }
